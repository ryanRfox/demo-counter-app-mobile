...
# optin application
def optin_app(client, private_key, index, app_args) : 
    # declare sender
    ...
    app_optin_txn = transaction.ApplicationOptInTxn(sender, params, app_id, app_args)

    # sign transaction
    signed_app_optin_txn = app_optin_txn.sign(private_key)
    tx_id = signed_app_optin_txn.transaction.get_txid()

    # send transactions
    client.send_transactions([signed_app_optin_txn])

    # await confirmation
    wait_for_confirmation(client, tx_id, 5)

    print("Application optin successful")
...
def main() :
    ...
    print("--------------------------------------------")
    print("Optin to Application ({})......".format(app_id))
    optin_app(algod_client, creator_private_key, app_id, None)
